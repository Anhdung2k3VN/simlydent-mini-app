...
type Props = {
  // Nếu bạn muốn vẫn truyền từ ngoài vào (khi mở từ list), cho phép optional:
...
const MedicalRecordDetailPage: React.FC<Props> = ({
  ...
  return (
    <div className="relative flex flex-col h-auto min-h-screen w-full overflow-x-hidden pb-24 bg-background font-display text-text-primary antialiased selection:bg-primary/30">
      {/* Header */}
      <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
      ...
      <main className="flex-1 w-full max-w-lg mx-auto p-4 flex flex-col gap-6">
        {/* Summary Card */}
        <div className="flex flex-col gap-4 rounded-xl bg-surface p-4 shadow-sm border border-slate-100 dark:border-slate-800">
        ...
        {/* Services */}
        <section>
          <h3 className="text-text-primary text-lg font-bold leading-tight px-1 mb-3 flex items-center gap-2">
          ...
          <div className="bg-surface rounded-xl overflow-hidden shadow-sm border border-slate-100 dark:border-slate-800 divide-y divide-slate-100 dark:divide-slate-800">
        ...
        <section>
          ...
          <div className="bg-surface rounded-xl p-4 shadow-sm border border-slate-100 dark:border-slate-800">
            <div className="flex gap-3">
              <div className="w-1 rounded-full bg-amber-400 shrink-0 h-auto" />
              <div className="flex flex-col gap-1">
                <h4 className="text-sm font-bold text-text-primary uppercase tracking-wider opacity-70">
                  Chẩn đoán
                </h4>
        ...
        <section>
          ...
          <div className="bg-surface rounded-xl p-5 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col gap-4">
              ...
              return (
                <div key={idx} className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <ul className="list-disc list-outside pl-4 text-sm text-text-secondary mt-1 space-y-1">
                      {p.bullets.map((b, i) => (
                        <li key={i}>{b}</li>
                      ))}
                    </ul>
      ...
      {/* Bottom Actions */}
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-surface border-t border-slate-200 dark:border-slate-800 p-4 pb-8 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
...
// ---- demo mock, bạn thay bằng call API ----
